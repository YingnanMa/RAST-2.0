...
parser.add_argument('--style_diff_weight', type=float, default=2.0)
parser.add_argument('--content_restoration_weight', type=float, default=1.0)
parser.add_argument('--style_restoration_weight', type=float, default=1.0)
...
valid = 1
fake = 0
D1 = net_v19.MultiDiscriminator()
D1.to(device)

D2 = net_v19.MultiDiscriminator()
D2.to(device)

vgg.load_state_dict(torch.load(args.vgg))
vgg = nn.Sequential(*list(vgg.children())[:44])
network = net_v19.Net(vgg, decoder, args.start_iter, args.breakpoint_transformer, args.breakpoint_decoder)
network.train()
network.to(device)
...
def draw_pic(loss_lst1,loss_lst2,loss_lst4,loss_lst5,loss_lst6,loss_lst7):
    ...
    plt.legend(fontsize=14)
    plt.savefig("/content/drive/Shareddrives/Style-Transfer/IEContraAST-main/Loss-V31.png")
...
#====================================================
filename = '/content/drive/Shareddrives/Style-Transfer/IEContraAST-main/loss_v31.xlsx'
columns=["content_transitive","style_transitive","style_difference","content_restoration","style_restoration","loss_identity1",
        "loss_identity2","loss_contrastive_c","loss_contrastive_s","loss_gan_g1","loss_gan_d1","loss_gan_g2","loss_gan_d2",
        "total_loss"]
...
for i in tqdm(range(args.start_iter, args.max_iter)):
    ...
    output1, rc1, rs1, output2, rc2, rs2, l_identity1, l_identity2, loss_contrastive_c, loss_contrastive_s, content_transitive_loss, style_transitive_loss, style_diff_loss, content_restoration_loss, style_restoration_loss= network(content_images, style_images, args.batch_size)
